...
const ManuePage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await fetch(
          `http://localhost:300/api/allitem?page=${currentPage}&size=${cardPerPage}&filter=${filter}&brand=${brand}&sort=${sort}&search=${search}`
        );
  ...
  useEffect(() => {
    const fetchDataCount = async () => {
      ...
      try {
        const res = await fetch(
         `http://localhost:300/api/itemcounts?filter=${filter}&brand=${brand}&search=${search}`
        );
        const data = await res.json();
